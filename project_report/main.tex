...
Automated job role prediction systems can improve efficiency in recruitment processes by quickly identifying suitable candidates. These systems can provide career guidance to job seekers by suggesting appropriate roles based on their profiles, while reducing human bias during initial screening. Additionally, they enable scalable matching for large-scale recruitment platforms, addressing the growing need for efficient candidate-job matching in the technology industry.
...
Precision-Recall curves were favored over ROC curves for per-class analysis, as ROC curves can provide overly optimistic assessments due to class imbalance \cite{saito_prec_recall}. ROC curves plot True Positive Rate (TPR) against False Positive Rate (FPR), and when the negative class is much larger than the positive class (common in multi-class problems), the FPR can remain low even with many false positives, making the ROC curve appear better than it actually is. Precision-Recall curves plot Precision versus Recall and are more sensitive to class imbalance, providing a more realistic assessment of model performance for small sized classes.
...
% Feature importance analysis using XGBoost's built-in importance scores reveals that skill embeddings contribute most significantly to predictions, followed by the field of study, experience level, and the degree level. This aligns with our intuition, as technical skills are the primary differentiator between job roles. A Data Scientist role, for example, can often be characterized by skills like ``Python'', ``Machine Learning'', and ``TensorFlow'', while a Frontend Developer role will be characterized by skills like ``JavaScript'', ``React'', and ``CSS''. The high importance of skill embeddings reflects their discriminative power.

% The field of study rank (e.g., ``Computer Science'' vs. ``Data Science'' vs. ``Business Administration'') rank second in importance, indicating that it provides valuable information for role prediction. This makes sense because certain roles (e.g., Data Scientist) are more likely associated with specific educational backgrounds (e.g., Statistics, Data Science, Computer Science with ML focus), while other roles (e.g., Software Engineer) might accept a broader range of educational backgrounds. The experience level has moderate importance, reflecting that while experience is relevant, it alone is not sufficient for accurate role prediction. The degree level has the lowest importance, suggesting that the specific field of study is more discriminative than the actual level of education.
...
show near-perfect performance (Average Precision $>$ 0.99), with individual class curves achieving an
Average Precision score above 0.98 across all thresholds, demonstrating robustness is threshold independent.
...
Several limitations should be be noted in order to properly assess the capabilities and scope of our model.

The dataset size is one such limitation. With 1,000 samples across 20 classes, the dataset is relatively small by modern machine learning standards, which often contain tens or hundreds of thousands of samples. While the model achieves excellent performance on this dataset, larger datasets would allow for a more through testing and potentially reveal additional patterns or edge cases. A larger dataset could allow for a more sophisticated evaluation, such as using temporal (how role requirements change over time) or geographic (location requirements for jobs) information.
...
Even among the classes that were retained, there is variation in sample sizes, with some roles having 50+ samples while others have closer to 40. While the model achieves high performance across all classes, the performance might be less consistent if the dataset distribution changes. This is particularly relevant for deployment scenarios where the distribution of candidates might differ.
...
\begin{itemize}
    \item \textbf{Adrean Cajigas}: Organized and coordinated discussion within the team. Set up meetings and goals for different aspects of the project. Helped create the base classes for the prediction pipeline and assisted in system integration testing.
    ...
    \item \textbf{Shayan Khoshkeifi}: Performed RandomizedSearchCV for hyperparameter optimization. Defined the search space for learning rates and tree depth to evaluate speed and accuracy.
    ...
    \item \textbf{Ethan Liu}: Compiled the final project report and documentation. Wrote the methodology and results sections, managed the bibliography, and performed final quality assurance testing on the completed model.
\end{itemize}
