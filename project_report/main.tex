...
Classes with fewer than 3 samples were filtered out to ensure reliable stratified splitting, as it requires at least 2 samples per class. This resulted in the removal of extremely rare job roles such as ``Finance'' and ``Video Game Designer'' which had only 1-2 samples each. While this reduces the overall number of classes slightly, it ensures that the remaining classes have sufficient representation for both training and evaluation.
